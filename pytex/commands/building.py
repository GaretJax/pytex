...
class Compile(Command):
    ...
    def execute(self, args):
        ...
        if success and recompile:
            ...
            self.logger.info(
                "Compiling document with {} support".format(support)
            )
        ...
        if success:
            ...
            self.post_build(dest)

    def get_nomencl_version(self, tempdir, master):
        try:
            with open(os.path.join(tempdir, "{}.nlo".format(master)), "rb") as fh:
    ...
    def parser(self):
        ...
        parser.add_argument(
            "--bibtex", "-b", action="store_const", const=True, default=False
        )
        parser.add_argument(
            "--glossary", "-g", action="store_const", const=True, default=False
        )
    ...
    def compile_bib(self, tempdir, master):
        ...
        try:
            if tempdir.startswith(base):
                ...
                exclude = tempdir
            for f in find_files_of_type(
                os.path.realpath("."), ("bib",), (exclude,)
            ):
                self.logger.debug(
                    "Copying bib {!r} file to build dir".format(f)
                )
    ...
    def compile(self, tempdir, master, dest=None, runs=1, defs=None):
        ...
        if defs:
            ...
            cmd += [r"{}\input{{master}}".format("".join(defs))]
        else:
            cmd += [
                "{}.tex".format(master),
            ]
    ...
    def post_build(self, path):
        cmd = self.config.get("compilation", "post-build")
        if not cmd:
            return
...
class Watch(Compile):
    ...
    name = "watch"
    help = (
        "Monitor the current directory for changes and rebuild the "
    ...
    def execute(self, args):
        ...
        def handler(event):
            ...
            if ignore_regex_str and ignore_regex.search(relative):
                self.logger.debug(
                    "Ignoring {!r} because of ignore regex "
            ...
            if isinstance(event, monitor.base.FileCreated):
                if event.path.endswith(".tex"):
                    ...
                    if not os.path.exists(builddir):
                        ...
                        os.mkdir(builddir)
                        self.logger.info(
                            "Added subdirectory {!r} to the "
                            "build directory".format(subdir)
                        )

            if event.path.endswith(".bib"):
                self.logger.info(
                    "Change detected at {!r}, recompiling with "
                ...
                self.logger.info(
                    "Change detected at {!r}, recompiling...".format(relative)
                )
                nomencl = self.get_nomencl_version(tempdir, master)
                success = self.compile(tempdir, master, defs=defs)
                if success and nomencl != self.get_nomencl_version(
                    tempdir, master
                ):
                    self.logger.info(
        ...
        if args.initial:
            self.logger.info("Compiling initial version...")
            self.compile(tempdir, master, dest, defs=defs)
...
class Clean(Command):
    ...
    name = "clean"
    help = (
        "Clean the document by deleting all files resulted from the "
        "building process."
    )
