...
class FindUnreferencedAcronyms(Command):
        ...
        for f in files:
            with open(f) as fh:
                ...
                for m in re.finditer(self.acronym_regex, contents):
                    ...
                    self.print_token_color(
                        stdscr, f, token, line, char, ms, me, prefix, postfix
                    )
    ...
    def replace_lower(self, content, start, end):
        self.replacements.append(
            (start, end, "\\gls{{{}}}".format(content[start:end].lower()))
        )
    ...
    def replace_lower_plural(self, content, start, end):
        ...
        if name.endswith("s"):
            name = name[:-1]
        self.replacements.append(
            (start, end, "\\glspl{{{}}}".format(name.lower()))
        )
    ...
    def print_token_color(
        self, stdscr, path, token, line, char, start, end, prefix, postfix
    ):
        abbr = token[start:end]
        path_prefix = ":{}:{}-{}".format(
            line + 1, char + 1, char + end - start
        )
